...
use super::{stitch_list::StitchSources, stitch_tables::{BASIC_REPORT_ITEMS, PROC_OPER_REPORT_ITEMS}, method_stats_reporter::POReportItems};
use std::mem;
    ...
    pub fn build(mut stitch_list: StitchList) -> Self {
        ...
        POReportItems::get_keys(&data)
            ...
            .for_each(|po_key| {
                ...
                let stitched_set = PROC_OPER_REPORT_ITEMS.0
                    .iter()
                    .map(|por| por.extract_stitched_line(&key_data))
                    .collect();
                stitched.process_operation.push((po_key.to_string(), stitched_set))
            });
