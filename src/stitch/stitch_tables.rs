use super::{
    ...
    key::Key,
    method_stats_reporter::{POReportItems, POReportItem, MethodStatsReporter},
...
lazy_static! {
    pub static ref PROC_OPER_REPORT_ITEMS: POReportItems = POReportItems(vec![
        ...
        POReportItem::new("Occurance percentage", |&(msv, _, num_traces)| Some(msv.count as f64 / num_traces as f64)),
        POReportItem::new("rate (avg)", |&(msv, num_files, _)| msv.get_avg_rate(num_files)),
        POReportItem::new("min_millis", |&(msv, _, _)| Some(msv.get_min_millis())),
        POReportItem::new("median_millis", |&(msv, _, _)| Some(msv.get_median_millis())),
        POReportItem::new("avg_millis", |&(msv, _, _)| Some(msv.get_avg_millis())),
        POReportItem::new("max_millis", |&(msv, _, _)| Some(msv.get_max_millis())),
        POReportItem::new("frac_not_http_ok", |&(msv, _, _)| Some(msv.get_frac_not_http_ok())),
        POReportItem::new("frac_error_logs", |&(msv, _, _)| Some(msv.get_frac_error_log())),
        ]);
}
...
pub fn append_method_table(buffer: &mut Vec<String>, data: &Vec<Option<StatsRec>>) {
    ...
    keys.into_iter()
        .for_each(|key| reporter.append_report(key));
...
lazy_static! {
    pub static ref CALL_CHAIN_REPORT_ITEMS: Vec<CCReportItem> = vec![
        CCReportItem::new("count", |msv, _, _| Some(msv.count as f64)),
        CCReportItem::new("Occurance percentage", |msv, _, num_traces| Some(msv.count as f64 / num_traces as f64)),
        CCReportItem::new("rate (avg)", |msv, num_files, _| msv.get_avg_rate(num_files)),
        CCReportItem::new("min_millis", |msv, _, _| Some(msv.get_min_millis())),
        CCReportItem::new("median_millis", |msv, _, _| Some(msv.get_median_millis())),
        CCReportItem::new("avg_millis", |msv, _, _| Some(msv.get_avg_millis())),
        CCReportItem::new("max_millis", |msv, _, _| Some(msv.get_max_millis())),
        CCReportItem::new("frac_not_http_ok", |msv, _, _| Some(msv.get_frac_not_http_ok())),
        CCReportItem::new("frac_error_logs", |msv, _, _| Some(msv.get_frac_error_log())),
        ];
}
