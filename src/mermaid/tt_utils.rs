...
pub fn get_call_chain_prefix(service_oper: &str, call_chain_key: &str) -> String {
    ...
    let prefix = regex::Regex::new(&format!("^.*{}", esc_service_oper)).expect("Failed to create a regex");
...
pub fn mark_selected_call_chain(mut sog: ServiceOperGraph, call_chain_key: &str) -> ServiceOperGraph {
